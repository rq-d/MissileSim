...
# LANDATX = -15750.0
PRONAVGAIN = 2.4
pitch_pid = [0.01, 0.01, 0.1]   # <100m miss params
pitch_pid = [0.01, 0.01,0.5] # 25 meter miss params
pitch_pid = [0.01, 0.01,0.8]  # 5 meter miss params
...
def apply_thrust(thrust: Thrust, f: el.Force, p: el.WorldPos) -> el.Force:
    ...
    return f + el.SpatialForce(linear=p.angular() @ thrust_vector_body_frame * thrust * 0)
...
def pronav_setpoint(accel: ProNavSetpoint, p: el.WorldPos, v:el.WorldVel) -> ProNavSetpoint:
    ...
    return jax.lax.cond(
        # dist2target < 15000,    # condition to be met
        False,
        lambda _: accel,   # if true return value
        lambda _: jnp.array([0.0,0.0]),    # if false return value
        operand=None,
    )
...
# keeps target from falling through the floor
@el.map
def bounce(p: el.WorldPos, v: el.WorldVel) -> el.WorldVel:
    return jax.lax.cond(
        jax.lax.max(p.linear()[2], v.linear()[2]) < 0.0,    # condition to be met
        lambda _: el.SpatialMotion(linear=v.linear() * jnp.array([1.0, 1.0, -1.0]) * 0.001),   # if true return value
        lambda _: v,    # if false return value
        operand=None,
    )
...
def world(seed: int = 0) -> el.World:
        ...
        [
            el.Body(
                world_pos=el.SpatialTransform(
                    angular=euler_to_quat(jnp.array([0.0, -90, 0.0])),
                    linear=jnp.array([LANDATX+100, 0.0, 10000.0]),
                ),
                inertia=el.SpatialInertia(3.0, jnp.array([0.1, 1.0, 1.0])),
                world_vel=el.SpatialMotion(linear=jnp.array([0, 0.0, -100.0]))
            ),
            Rocket(),
    ...
    ball_mesh = w.insert_asset(el.Mesh.sphere(BALL_RADIUS))
    ...
    w.spawn(
        el.Panel.hsplit(
            ...
            el.Panel.vsplit(
                ...
                el.Panel.graph(el.GraphEntity(rocket, AngleOfAttack)),
            ),
            el.Panel.vsplit(
                el.Panel.graph(el.GraphEntity(rocket, ProNavSetpoint)),
                el.Panel.graph(el.GraphEntity(rocket, AccelSetpointSmooth)),
            ),
            el.Panel.vsplit(
